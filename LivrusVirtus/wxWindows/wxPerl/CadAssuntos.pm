...
sub new {
    ...
    $Resource::true );
  $self->{labDescricao}->SetLabel('Assunto:');
...
sub OnBtnNovoClick {
  ...
  $self->btnNovoClick();
  $self->NovoDado($ConsSQL->Assunto('N', $self->{edtCodigo}->GetValue(),""));
  $event->Skip();
}
...
sub OnBtnSalvarClick {
  ...
  $self->SalvarDados($self->ValidaDados($Resource::true),
    $ConsSQL->Assunto('S', $self->{edtCodigo}->GetValue(),""),
    $ConsSQL->Assunto('U', $self->{edtCodigo}->GetValue(), 
    $self->{edtDescricao}->GetValue()),
    $ConsSQL->Assunto('I', $self->{edtCodigo}->GetValue(), 
...
sub OnBtnExcluirClick {
  ...
  $self->ExcluirDados($self->ValidaDados($Resource::false),
    $ConsSQL->Assunto('D', $self->{edtCodigo}->GetValue(),""));
  $event->Skip();
}
