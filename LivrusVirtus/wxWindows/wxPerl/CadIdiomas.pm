...
sub OnBtnNovoClick {
  ...
  $self->btnNovoClick();
  $self->NovoDado($ConsSQL->Idioma('N', $self->{edtCodigo}->GetValue(),""));
  $event->Skip();
}
...
sub OnBtnSalvarClick {
  ...
  $self->SalvarDados($self->ValidaDados($Resource::true),
    $ConsSQL->Idioma('S', $self->{edtCodigo}->GetValue(),""),
    $ConsSQL->Idioma('U', $self->{edtCodigo}->GetValue(), 
    $self->{edtDescricao}->GetValue()),
    $ConsSQL->Idioma('I', $self->{edtCodigo}->GetValue(), 
...
sub OnBtnExcluirClick {
  ...
  $self->ExcluirDados($self->ValidaDados($Resource::false),
    $ConsSQL->Idioma('D', $self->{edtCodigo}->GetValue(),""));
  $event->Skip();
}
