...
sub OnBtnNovoClick {
  ...
  $self->btnNovoClick();
  $self->NovoDado($ConsSQL->Profissao('N', $self->{edtCodigo}->GetValue(),""));
  $event->Skip();
}
...
sub OnBtnSalvarClick {
  ...
  $self->SalvarDados($self->ValidaDados($Resource::true),
    $ConsSQL->Profissao('S', $self->{edtCodigo}->GetValue(),""),
    $ConsSQL->Profissao('U', $self->{edtCodigo}->GetValue(), 
    $self->{edtDescricao}->GetValue()),
    $ConsSQL->Profissao('I', $self->{edtCodigo}->GetValue(), 
...
sub OnBtnExcluirClick {
  ...
  $self->ExcluirDados($self->ValidaDados($Resource::false),
    $ConsSQL->Profissao('D', $self->{edtCodigo}->GetValue(),""));
  $event->Skip();
}
