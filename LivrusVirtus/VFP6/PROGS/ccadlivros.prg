...
if Rotinas.ValidaCampo(frmCad.edtCodigo.Value, "ISBN não informado!")
  return .F.
Endif
if bTodosDados
  if Rotinas.ValidaCampo(frmCad.edtDescricao.Value, "Título não informado!")
	 return .F.
  Endif
  if Rotinas.ValidaCampo(frmCad.edtEditora.Value, "Editora não informada!")
	 return .F.
  Endif
  if lstAssuntos.Items.Count = 0
     MESSAGEBOX("Assunto(s) não informado(s)!", 16, _Screen.Caption)
	 return .F.
  Endif
  if lstAutores.Items.Count = 0
     MESSAGEBOX("Autore(s) não informado(s)!", 16, _Screen.Caption)
	 return .F.
  Endif
Endif
EndFunc
...
if alltrim(frmCad.edtCodEditora.Value)<>""   
  If Rotinas.ValidaInteger(alltrim(frmCad.edtCodEditora.Value)) = .T.
    ...
    sMensagemErro = "Editora não encontrada!"
...
if alltrim(frmCad.edtCodAssunto.Value)<>""   
  If Rotinas.ValidaInteger(alltrim(frmCad.edtCodAssunto.Value)) = .T.
    ...
    sMensagemErro = "Assunto não encontrado!"
...
if alltrim(frmCad.edtCodAutor.Value)<>""   
  If Rotinas.ValidaInteger(alltrim(frmCad.edtCodAutor.Value)) = .T.
    ...
    sMensagemErro = "Autor não encontrado!"
