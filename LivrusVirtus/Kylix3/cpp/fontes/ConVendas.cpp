...
if (sOpcao=='L')
 ...
 return ConsSQL->Venda(sOpcao,edtISBN->Text.c_bstr(),edtDescricao->Text.c_bstr(),
 sDtHrVenda.Trim(),"","");
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmConVendas::edtDescricaoExit(TObject *Sender)
{
 if (edtDescricao->Text!="") {
   if (Rotinas->ValidaCPF(edtDescricao->Text.c_bstr())) {
      if (Rotinas->ConsultaDados(ConsCampo,
         ConsSQL->Cliente('S',edtDescricao->Text.c_bstr(),"","","","","","","","","")))
...
void __fastcall TfrmConVendas::edtISBNExit(TObject *Sender)
{
 if (edtISBN->Text!="") {
   if (Rotinas->ValidaISBN(edtISBN->Text.c_bstr())) {
      if (Rotinas->ConsultaDados(ConsCampo,ConsSQL->Livro('Q',edtISBN->Text.c_bstr(),
         "","","","","","","","",""))) {
         if (ConsCampo->Fields->Fields[3]->AsInteger > 0) {
            ...
            MessageDlg("Livro não existe no estoque!", mtError,
            TMsgDlgButtons() << mbOk, 0);
      } else
