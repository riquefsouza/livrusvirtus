{------------------------------------------------------------------------------
 ...
 Versão programada na linguagem Delphi 8.0 for the Microsoft .NET.
------------------------------------------------------------------------------}
program Livrus;
...
uses
  SysUtils, System.IO, System.Text;
...
var
 ...
 tfLivro, tfLivroAss, tfLivroAut, tfCliente, tfUsuario, tfVenda: FileStream;

{--- Rotinas básicas ---------------------------------------------------------}
function trim(sTexto: string): string; forward;
function repete(sTexto: string; nQtd: integer): string; forward;
function esp(sTexto: string; nTam: integer):string; forward;
function zeros(stexto:string; ntam:integer): string; forward;
function strRelativa(sTexto, sChave: string):boolean; forward;
function modulo11(svalor: string; ndig: integer):integer; forward;
function validaCPF(cpf: string): boolean; forward;
function validaISBN(isbn: string): boolean; forward;
function digitaSenha(sRotulo: string; nTamSenha: integer): string; forward;
function validaNumero(sRotulo, sNumero: string): string; forward;
function validaDataHora(sRotulo, sDataHora: string;
         bSoData: boolean): string; forward;
function retDataHora: string; forward;

{--- Rotinas de manipulação de arquivos --------------------------------------}
function tamArq(var sArq: FileStream): integer; forward;
function fimArq(var sArq: FileStream): Boolean; forward;
function lerArq(var sArq: FileStream; nTamLinha,
         nTamCRLF: integer): string; forward;
procedure abrecriaArqs(bAbre: boolean); forward;
procedure fechaArqs; forward;
function pesqArq(var sArq: FileStream; sChave: string; nPosChave,
		nTamChave, nTamLinha: integer): string; forward;
procedure pesqLinhaExcluida(var sArq: FileStream; nTamLinha: integer); forward;
function pesqMaxCod(var sArq: FileStream; nPosChave,
         nTamChave, nTamLinha: integer): string; forward;
procedure escreveArq(var sArq: FileStream; sTexto, sMsgAcerto,
         sMsgErro: string); forward;

{--- Rotinas de implementação do sistema e de interface ----------------------}
function LinhaReg(sreg: ctArqs): string; forward;
function tamReg(sreg: ctArqs): integer; forward;
function rotulo(sreg: ctArqs; nNum: integer): string; forward;
function ContinuaSN(nlinha, nQtdLinha: integer): boolean; forward;
function ListarTodos(var sArq: FileStream; sreg: ctArqs; sChave: string;
          nPosChave, nTamChave, nTamLinha, nQtdLinha: integer;
          bPesqRelativa: boolean): boolean; forward;
procedure ListarRegLigados(var sArq1: FileStream; sreg1: ctArqs; nPosChave1,
          nTamChave1, nPosChave2, nTamChave2: integer; sChave: string;
      var sArq2: FileStream; sreg2: ctArqs; nPosChave3, nQtdLinha: integer); forward;
function validaEntrada(var sArq: FileStream; sreg: ctArqs; nTexto: integer;
        sEntrada: string; nTentativas: integer;
        bPesqRelativa: boolean; bTipoEntrada: ctTipo): string; forward;
...
procedure mostrarDados(sreg: ctArqs; slinha: string; nOpcao: integer); forward;
procedure mostrarSubDados(sreg, ssubreg: ctArqs; bSoMostrar: boolean); forward;
procedure frmCadLigados(sreg: ctArqs; sacao: ctAcao; sCodigo: string); forward;
procedure ExcluirRegLigados(var sArq: FileStream; sreg: ctArqs; nPosChave,
          nTamChave: integer; sChave: string); forward;
...
------------------------------------------------------------------------------}
function trim(sTexto: string): string;
...
------------------------------------------------------------------------------}
function repete(sTexto: string; nQtd: integer): string;
var ncont: integer;
    sret: string;
begin
sret:='';
...
------------------------------------------------------------------------------}
function esp(sTexto: string; nTam: integer):string;
begin
...
------------------------------------------------------------------------------}
function zeros(stexto:string; ntam:integer): string;
var ncont: integer;
    saux: string;
begin
  saux:='';
...
------------------------------------------------------------------------------}
function strRelativa(sTexto, sChave: string):boolean;
...
------------------------------------------------------------------------------}
function digitaSenha(sRotulo: string; nTamSenha: integer): string;
var sSenha: string;
{    sLetra: char;
    ncont: integer;}
begin
...
retorna - o sNumero
------------------------------------------------------------------------------}
function validaNumero(sRotulo, sNumero: string): string;
var nnum: double;
    nStatus: integer;
begin
nStatus:=-1;
...
retorna - a sDataHora
------------------------------------------------------------------------------}
function validaDataHora(sRotulo, sDataHora: string; bSoData: boolean): string;
...
------------------------------------------------------------------------------}
function tamArq(var sArq: FileStream): integer;
begin
  tamArq := sArq.Length;
end;
{------------------------------------------------------------------------------
...
------------------------------------------------------------------------------}
function fimArq(var sArq: FileStream): Boolean;
begin
...
------------------------------------------------------------------------------}
function lerArq(var sArq: FileStream; nTamLinha, nTamCRLF: integer): string;
var nLidos, ncont: integer;
 slinha: array[1..TAM_MAX_REG] of Byte;
 sret: string;
begin
sret := '';
//sLer: StreamReader := StreamReader.Create(sArq);
//sret:= sLer.ReadLine();
...
if nLidos <> (nTamLinha + nTamCRLF) then
  ...
  for ncont:=1 to (nLidos - nTamCRLF) do
    sret:=sret + Chr(slinha[ncont]);
end;
...
begin
if bAbre then begin
  if System.IO.File.Exists(ARQ_ASSUNTO) then
    tfAssunto:=FileStream.Create(ARQ_ASSUNTO,FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_ASSUNTO);
  if System.IO.File.Exists(ARQ_AUTOR) then
    tfAutor:=FileStream.Create(ARQ_AUTOR, FileMode.Open, FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_AUTOR);
  if System.IO.File.Exists(ARQ_EDITORA) then
    tfEditora:=FileStream.Create(ARQ_EDITORA,FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_EDITORA);
  if System.IO.File.Exists(ARQ_IDIOMA) then
    tfIdioma:=FileStream.Create(ARQ_IDIOMA, FileMode.Open, FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_IDIOMA);
  if System.IO.File.Exists(ARQ_ENDERECO) then
    tfEndereco:=FileStream.Create(ARQ_ENDERECO,FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_ENDERECO);
  if System.IO.File.Exists(ARQ_PAIS) then
    tfPais:=FileStream.Create(ARQ_PAIS, FileMode.Open, FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_PAIS);
  if System.IO.File.Exists(ARQ_PROFISSAO) then
    tfProfissao:=FileStream.Create(ARQ_PROFISSAO,FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_PROFISSAO);
  if System.IO.File.Exists(ARQ_LIVRO) then
    tfLivro:=FileStream.Create(ARQ_LIVRO, FileMode.Open, FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_LIVRO);
  if System.IO.File.Exists(ARQ_LIVASS) then
    tfLivroAss:=FileStream.Create(ARQ_LIVASS, FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_LIVASS);
  if System.IO.File.Exists(ARQ_LIVAUT) then
    tfLivroAut:=FileStream.Create(ARQ_LIVAUT, FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_LIVAUT);
  if System.IO.File.Exists(ARQ_CLIENTE) then
    tfCliente:=FileStream.Create(ARQ_CLIENTE, FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_CLIENTE);
  if System.IO.File.Exists(ARQ_USUARIO) then
    tfUsuario:=FileStream.Create(ARQ_USUARIO, FileMode.Open,FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_USUARIO);
  if System.IO.File.Exists(ARQ_VENDA) then
    tfVenda:=FileStream.Create(ARQ_VENDA, FileMode.Open, FileAccess.ReadWrite)
  else
    System.Console.WriteLine(MSG_ARQERRO+' --> '+ARQ_VENDA);
end else begin
  tfAssunto:=FileStream.Create(ARQ_ASSUNTO,FileMode.Create,FileAccess.ReadWrite);
  tfAutor:=FileStream.Create(ARQ_AUTOR,FileMode.Create,FileAccess.ReadWrite);
  tfEditora:=FileStream.Create(ARQ_EDITORA,FileMode.Create,FileAccess.ReadWrite);
  tfIdioma:=FileStream.Create(ARQ_IDIOMA,FileMode.Create,FileAccess.ReadWrite);
  tfEndereco:=FileStream.Create(ARQ_ENDERECO,FileMode.Create,FileAccess.ReadWrite);
  tfPais:=FileStream.Create(ARQ_PAIS,FileMode.Create,FileAccess.ReadWrite);
  tfProfissao:=FileStream.Create(ARQ_PROFISSAO,FileMode.Create,FileAccess.ReadWrite);
  tfLivro:=FileStream.Create(ARQ_LIVRO,FileMode.Create,FileAccess.ReadWrite);
  tfLivroAss:=FileStream.Create(ARQ_LIVASS,FileMode.Create,FileAccess.ReadWrite);
  tfLivroAut:=FileStream.Create(ARQ_LIVAUT,FileMode.Create,FileAccess.ReadWrite);
  tfCliente:=FileStream.Create(ARQ_CLIENTE,FileMode.Create,FileAccess.ReadWrite);
  tfUsuario:=FileStream.Create(ARQ_USUARIO,FileMode.Create,FileAccess.ReadWrite);
  tfVenda:=FileStream.Create(ARQ_VENDA,FileMode.Create,FileAccess.ReadWrite);
end;
...
------------------------------------------------------------------------------}
function pesqArq(var sArq: FileStream; sChave: string; nPosChave,
		nTamChave, nTamLinha: integer): string;
var
 sres, slinha: string;
 bQuebra: Boolean;
 nPosicao: integer;
begin
  ...
  while (not fimArq(sArq)) and (bQuebra = false) do begin
...
nTamLinha - o tamanho do registro
------------------------------------------------------------------------------}
procedure pesqLinhaExcluida(var sArq: FileStream; nTamLinha: integer);
...
------------------------------------------------------------------------------}
procedure escreveArq(var sArq: FileStream; sTexto, sMsgAcerto, sMsgErro: string);
var
  slinha: array[1..TAM_MAX_REG] of Byte;
  ntam, ncont: integer;
begin
  try
    //sGravar: StreamWriter := StreamWriter.Create(sArq);
    sTexto := sTexto + CRLF;
    ntam := Length(sTexto);
    for ncont:=1 to ntam do
    	slinha[ncont]:=ord(sTexto[ncont]);
...
retorna - o rotulo
------------------------------------------------------------------------------}
function rotulo(sreg: ctArqs; nNum: integer): string;
var sret: string;
begin
sret:='';
...
if nlinha > 0 then begin
  if (nlinha mod nQtdLinha) = 0 then begin
      ...
      while(bSOp) do begin
       menu(13);
       opCSubMenu:=System.Console.In.ReadLine.Chars[0];
...
procedure ListarRegLigados(var sArq1: FileStream; sreg1: ctArqs; nPosChave1,
nTamChave1, nPosChave2, nTamChave2: integer; sChave: string;
var sArq2: FileStream; sreg2: ctArqs; nPosChave3, nQtdLinha: integer);
var slinha1: string;
...
while (not fimArq(sArq1)) and (bQuebra=false) do begin
...
bQuebra:=false;
  while (bQuebra=false) and (ncont <= nTentativas ) do begin
    if opTexto in bTipoEntrada then begin
      write(sTexto);
      sEntrada:=System.Console.In.ReadLine;
...
begin
 ...
 writeln('| Versao programada na linguagem Delphi 8.0 .NET.            |');
 writeln('*------------------------------------------------------------*');
...
------------------------------------------------------------------------------}
procedure ExcluirRegLigados(var sArq: FileStream; sreg: ctArqs;
    nPosChave, nTamChave: integer; sChave: string);
var
  slinha: string;
  sCodigo: string;
begin
  slinha:='';
  sArq.Seek(0, SeekOrigin.Begin);
  while (not fimArq(sArq)) do begin
