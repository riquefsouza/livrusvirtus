...
C     ------------------------------------------------------------------
C      Call GetArg(0, sarg)
C      if (sarg.eq.'-cria') then
C         call abrecriaArqs(.false.)
C      else
        call frmSplash
...
C     ------------------------------------------------------------------
      ...
      integer nvalor, nsoma, ncont, TamStr, StrToInt
...
C     ------------------------------------------------------------------
     ...
     *  sEntrada, nTentativas, bPesqRelativa, bTipoEntrada)
      ...
      character*255 sres, sTexto, sMsgErro, rotulo, ftrim
...
C     ------------------------------------------------------------------
      ...
      real nTotalPreco
      character*255 slinha, sQtd, rotulo,
...
C          sTotalPreco = FloatToStr(nTotalPreco)
          write(*,*) 'Total de ',rotulo(sreg,4), nTotalPreco
        end if  
      else if (sacao .eq. opExcluir) then
        if (sreg .eq. opLivroAss) then
