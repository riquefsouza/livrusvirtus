...
modulo11, validaCPF, validaISBN, digitaSenha, validaNumero, 
validaDataHora, retDataHora;
...
tamArq, lerArq, gravarArq, abrecriaArqs, fechaArqs,
pesqArq, pesqLinhaExcluida, pesqMaxCod, escreveArq,
ctArqs, ctAcao, ctTipo, tamReg, LinhaReg;
FROM Sistema IMPORT QTD_MAX_LINHA, MSG_ERRO, 
g_nPosChave, g_nTamChave, g_nPreco, rotulo, menu, 
ContinuaSN, ListarTodos, ListarRegLigados, validaEntrada, 
mostrarDados, mostrarSubDados, btnAcao, ExcluirRegLigados;

FROM CardStr IMPORT Value, Give, Alignment;
FROM RealStr IMPORT ConvResults, GiveFloat; 
FROM StdStrings IMPORT String, Length, Assign,
Append, Extract, Compare, CompareResult;
FROM UxFiles IMPORT SetPos;
FROM InOut IMPORT Read, ReadLn, WriteLn, ReadInt, WriteString;
...
WHILE (bQuebra=FALSE) DO 
  ...
  | 1: sSenhaAtual:=digitaSenha('Senha Atual: ',10);
       IF Compare(sSenhaAtual,tUsuario.senha)<>equal THEN 
  ...
  | 2: sNovaSenha:=digitaSenha('Nova Senha: ',10);
       ...
       IF Compare(sNovaSenha,sConfirmaSenha)<>equal THEN 
    ...
    WHILE nCont <= nQtd DO 
      ...
      IF Compare(slinha,'')<>equal THEN 
        ...
        IF validaISBN(tVenda.isbn) THEN 
          ...
          nQtdEstoque:=Value(tLivro.qtdestoque);
          IF nQtdEstoque > 0 THEN 
             ...
             IF g_SalvarInclusao THEN 
                ...
                IF Compare(slinha,'')<>equal THEN 
                   ...
                   Give(tLivro.qtdestoque,nQtdEstoque,10,left);
      ...
      END;
    END;
    GiveFloat(sTotalPreco, nTotalPreco, 7, 10, left);
