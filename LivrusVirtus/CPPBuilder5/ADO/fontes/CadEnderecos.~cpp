...
if (Rotinas->ValidaCampo(edtCodigo->Text,"Código não informado!"))
  return false;
if (bTodosDados) {
  if (Rotinas->ValidaCampo(edtDescricao->Text,"Logradouro não informado!"))
    return false;
  if (Rotinas->ValidaCampo(edtBairro->Text,"Bairro não informado!"))
    return false;
  if (Rotinas->ValidaCampo(edtCEP->Text,"CEP não informado!"))
    return false;
  if (Rotinas->ValidaCampo(edtCidade->Text,"Cidade não informada!"))
    return false;
}
return true;
}
