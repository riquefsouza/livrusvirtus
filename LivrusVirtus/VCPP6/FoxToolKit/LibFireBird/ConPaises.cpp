...
long CfrmConPaises::onBtnPesquisar(FXObject* obj,FXSelector sel,void* ptr) {
  PesquisarDados(
    ConsSQL->Pais('S',"", edtDescricao->getText()), STR_PAISNENC, 2);
  return true;
}

long CfrmConPaises::onBtnLimpar(FXObject* obj,FXSelector sel,void* ptr) {
  LimparDados();
  return true;
}

long CfrmConPaises::onGridSelected(FXObject* obj,FXSelector sel,void* ptr){
  FXTableRange *escala = (FXTableRange*) ptr;
  setLabRegistros(escala->fm.row);
  return true;
}

long CfrmConPaises::onGridChanged(FXObject* obj,FXSelector sel,void* ptr){
