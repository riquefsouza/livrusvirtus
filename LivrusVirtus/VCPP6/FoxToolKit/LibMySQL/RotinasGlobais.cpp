...
FXbool CRotinasGlobais::ConectaBanco(FXString sBancoDeDados, FXString sLogin, 
  FXString sSenha) {
  FXString sTemp;
  ...
  if ( gConexao && 
       mysql_real_connect( gConexao, "localhost", 
         sLogin.text(), sSenha.text(), NULL, MYSQL_PORT, NULL, 0 ) )
  {
    if ( mysql_select_db( gConexao, sBancoDeDados.text() ) < 0 ) {
    ...
    sTemp = FXStringFormat("%s: %d", STR_NCONPORTA, MYSQL_PORT);
...
FXbool CRotinasGlobais::ConsultaDados(CONEXAO mConLocal, FXString sSql)
{
FXbool bRet;
MYSQL_RES	*mResultado;
  
  if ( mysql_query(mConLocal, sSql.text()) == 0 ) {
...
FXbool CRotinasGlobais::ConsultaDadosLista(CONEXAO mConLocal, 
    ...
    while ( mLinha ) {
      ...
      for (nCol = 0; nCol < nQtdCol; nCol++) {	  
        if (mCampo[nCol].type == FIELD_TYPE_DATETIME)
          sLinha[nCol] = Rotinas->ConverteDataHoraStr(mLinha[nCol], false, 
            "%d/%m/%Y","%H:%M:%S");
        else
...
FXbool CRotinasGlobais::AtualizaDados(CONEXAO mConLocal, FXString sSql)
{ 
  if ( mysql_query(mConLocal, sSql.text()) == 0)
    return true;
  else {
