...
LPSTR sConexaoInfo;
  ...
  sConexaoInfo = Rotinas_alocaString(TAM_MAX_STR);

  wsprintf(sConexaoInfo, "dbname = %s user = %s password = %s",
...
BOOL Rotinas_ConsultaDados(CONEXAO pgConLocal, LPTSTR sSql)
{
  BOOL bRet;
  PGresult *pgResultado;
  LPTSTR sTemp;

  pgResultado = PQexec(pgConLocal, sSql);
  if (PQresultStatus(pgResultado) == PGRES_TUPLES_OK) {
    ...
    wsprintf(sTemp, "%s - %s", Rotinas_lerStr(STR_NCEXECSQL), 
...
BOOL Rotinas_AtualizaDados(CONEXAO pgConLocal, LPTSTR sSql)
{
  BOOL bRet;
  PGresult *pgResultado;
  LPTSTR sTemp;
...
BOOL Rotinas_ConsultaDadosLista(CONEXAO pgConLocal, LISTAMSTR *lstLocal, LPTSTR sSql)
{
  ...
  for (nLinha = 0; nLinha < PQntuples(pgResultado); nLinha++)
  {
    for (nCol = 0; nCol < nQtdCol; nCol++) {
      ...
      if (PQftype(pgResultado, nCol) == 1114) //Timestamp
        sLinha[nCol] = Rotinas_ConverteDataHoraStr(sLinha[nCol], 
