...
BOOL Rotinas_ConectaBanco(LPTSTR sFonteDeDados, LPTSTR sLogin, LPTSTR sSenha)
{
SQLRETURN rc;
  ...
  if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
    ...
    if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
      ...
      rc = SQLConnect(hdbc, (SQLTCHAR*) sFonteDeDados, SQL_NTS,
               (SQLTCHAR*) sLogin, SQL_NTS, (SQLCHAR*) sSenha, SQL_NTS);
...
BOOL Rotinas_ConsultaDados(CONEXAO  hstmtlocal, LPTSTR sSql)
{
SQLRETURN   rc;
...
VOID Rotinas_arrumaColunas(CONEXAO  hstmtlocal, LPTSTR sLinha[],
...
UDWORD    nColPrecisao;
   ...
   for (nCol = 0; nCol < nQtdCol; nCol++) {	
      ...
      lstrcpy(sLinha[nCol], Rotinas_ConverteDataHoraStr(sLinha[nCol], FALSE, 
        "dd/MM/yyyy", "HH:mm:ss"));
	 }
	  /*
	 else if (nColTipo == SQL_DECIMAL || nColTipo == SQL_DOUBLE 
		 || nColTipo == SQL_FLOAT)
		 sLinha[nCol] = Rotinas_VirgulaParaPonto(sLinha[nCol], FALSE);*/
   }
}

BOOL Rotinas_ConsultaDadosLista(CONEXAO  hstmtlocal, LISTAMSTR *lstLocal, LPTSTR sSql)
{
...
BOOL Rotinas_AtualizaDados(CONEXAO  hstmtlocal, LPTSTR sSql)
{
SQLRETURN   rc;
