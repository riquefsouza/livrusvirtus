...
BOOL Rotinas_ConectaBanco(LPTSTR sStrConexao)
{
  ...
  try 
  {
    if FAILED( gConexao.CreateInstance("ADODB.Connection") ) {
      MessageBox(0, Rotinas_lerStr(STR_NCCINST), "Erro", MB_ICONERROR | MB_OK);
      return FALSE;
    }
  ...
  catch (_com_error &e)
  {
    ...
    wsprintf(sTemp, "%s - %s", Rotinas_lerStr(STR_NCONBANCO), 
