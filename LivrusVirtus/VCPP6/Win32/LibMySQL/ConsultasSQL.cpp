...
if (sTipo=='N')
  ...
  lstrcpy(sCons, "select a.codautor as Código, a.Autor as Autor from Autor a");
...
if (sTipo=='N')
  ...
  lstrcpy(sCons, "select e.codeditora as Código, e.editora as Editora from editora e");
...
if (sTipo=='N')
  ...
  lstrcpy(sCons, "select i.codidioma as Código, i.idioma as idioma from Idioma i");
...
if (sTipo=='S') {
  lstrcpy(sCons, "select p.codpais as Código, p.pais as País from Pais p");
...
if ((sTipo=='S') || (sTipo=='P')) {
  if (sTipo=='S')
    ...
    wsprintf(sCons, "%s%s%s", "select sum(l.preco) as PreçoTotal",
    " from Venda v, Livro l, Cliente c",
    " where v.isbn=l.isbn and v.cpf=c.cpf");
  if (lstrlen(sISBN) > 0)
