...
{ wxString sTemp;
         ...
         sLogin.c_str(), sSenha.c_str(), NULL, MYSQL_PORT, NULL, 0 ) )
  {
    ...
    sTemp = wxString::Format("%s: %d", STR_NCONPORTA, MYSQL_PORT);
...
bool CRotinasGlobais::ConsultaDados(CONEXAO mConLocal, wxString sSql)
{
bool bRet;
...
bool CRotinasGlobais::AtualizaDados(CONEXAO mConLocal, wxString sSql)
{ 
  if ( mysql_query(mConLocal, sSql.c_str()) == 0)
    return TRUE;
  else {
