...
DbConnectInf = new wxDbConnectInf(NULL, sFonteDeDados, sLogin, sSenha);
  ...
  gConexao = wxDbGetConnection(DbConnectInf);
  if (gConexao == 0) 
    bret = false;
  else
    bret = true;     
...
bool CRotinasGlobais::AtualizaDados(CONEXAO dbBanco, wxString sSql) {  
bool bret = false;
...
bool CRotinasGlobais::ConsultaDados(CONEXAO dbBanco, wxString sSql) {  
SDWORD cb;
...
bool CRotinasGlobais::ConsultaDadosLista(CONEXAO dbBanco, 
...
wxString sret = "";
  ...
  if (ConsultaDadosLista(gConexao, Campos, sTextoSql, nQtdCol)) {
...
{ 
  ...
  if (ConsultaDadosLista(gConexao, &Campos, sTextoSql, 2)) {
