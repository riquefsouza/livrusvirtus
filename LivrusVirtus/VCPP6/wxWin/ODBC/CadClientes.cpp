...
void CfrmCadClientes::IniciaControles() {
  ...
  labDtNasc = new wxStaticText(this, ID_FRMCADCLIENTES_LABDTNASC,
    ...
    edtDtNasc->Connect(edtDtNasc->GetId(), wxEVT_KILL_FOCUS, 
      (wxObjectEventFunction) (wxEventFunction)
...
{ 
  if (bInformar) {
    ...
    #if defined(__WXMSW__) || defined(__WXX11__)
      edtDtNasc->SetDataStr(
        lstRegistros.col[6].Item(nLinhaRegistro).Mid(0,10),true);
    #else
      edtDtNasc->SetValue(lstRegistros.col[6].Item(nLinhaRegistro).Mid(0,10));
    #endif
...
void CfrmCadClientes::OnEdtDtNasc_Killfocus(wxFocusEvent& event)
{ wxTextCtrl *mData;
...
{ wxString sLogradouro;
  ...
  if (m_EdtCodEnd->GetValue().Len() > 0) {
    ...
    m_EdtCodEnd->GetValue(),"","","","",""),STR_ENDNENC);
...
{ wxString sPais;
  ...
  if (m_EdtCodPais->GetValue().Len() > 0) {
    sPais = Rotinas->ConsultaCampoDesc(&ConsCampo, ConsSQL->Pais('S',
      m_EdtCodPais->GetValue(),""),STR_PAISNENC);    
    if (sPais.Len() > 0) {
...
{ wxString sProfissao;
  ...
  if (m_EdtCodProfissao->GetValue().Len() > 0) {
    sProfissao = Rotinas->ConsultaCampoDesc(&ConsCampo, ConsSQL->Profissao('S',
      m_EdtCodProfissao->GetValue(),""),STR_PROFNENC);    
