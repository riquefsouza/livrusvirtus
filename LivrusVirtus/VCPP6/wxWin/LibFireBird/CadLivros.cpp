...
void CfrmCadLivros::IniciaControles() {
  ...
  edtEdicao->SetValidator(wxTextValidator(wxFILTER_NUMERIC, NULL));
...
int nCont, nQtd;
  ...
  if (sOpcao=='A') {
    Rotinas->AtualizaDados(gConexao, ConsSQL->LivroAss('D',sISBN,wxT(""))); 
    ...
    for (nCont=0; nCont < nQtd; nCont++)  {      
  	...
  	Rotinas->AtualizaDados(gConexao,ConsSQL->LivroAut('D',sISBN,wxT("")));
...
{ wxString sEditora;
  ...
  if (m_EdtCodEditora->GetValue().Len() > 0) {
    sEditora = Rotinas->ConsultaCampoDesc(&ConsCampo, ConsSQL->Editora('S',
      m_EdtCodEditora->GetValue(),""),STR_EDTNENC);    
    if (sEditora.Len() > 0) {
...
{ wxString sAssunto;
  ...
  if (m_EdtCodAssunto->GetValue().Len() > 0) {
    sAssunto = Rotinas->ConsultaCampoDesc(&ConsCampo, ConsSQL->Assunto('S',
      m_EdtCodAssunto->GetValue(),""),STR_ASSNENC);    
    if (sAssunto.Len() > 0) {
...
{ wxString sAutor;
  ...
  if (m_EdtCodAutor->GetValue().Len() > 0) {
    sAutor = Rotinas->ConsultaCampoDesc(&ConsCampo, ConsSQL->Autor('S',
      m_EdtCodAutor->GetValue(),""),STR_AUTNENC);    
    if (sAutor.Len() > 0) {
