...
bool CRotinasGlobais::ConectaBanco(wxString sBancoDeDados, wxString sLogin, wxString sSenha)
{
wxString sConexaoInfo;
  
  sConexaoInfo = wxString::Format("dbname = %s user = %s password = %s",
    sBancoDeDados, sLogin, sSenha);

  gConexao = PQconnectdb(sConexaoInfo.c_str());
  if (PQstatus(gConexao) != CONNECTION_OK)
  {
    //fprintf(stderr, "Conexão com o banco de dados falhou: %s",
    //        PQerrorMessage(gConexao));
    wxMessageBox(STR_NCONBANCO, wxT("Erro"), wxOK | wxICON_ERROR);
    PQfinish(gConexao);
    return false;
  }
return true;
...
bool CRotinasGlobais::ConsultaDados(CONEXAO pgConLocal, wxString sSql)
{
  bool bRet;
  ...
  if (PQresultStatus(pgResultado) == PGRES_TUPLES_OK) {
    ...
    sTemp = wxString::Format("%s - %s", STR_NCEXECSQL, 
...
bool CRotinasGlobais::AtualizaDados(CONEXAO pgConLocal, wxString sSql)
{
  bool bRet;
  PGresult *pgResultado;
  wxString sTemp;
  ...
  for (nLinha = 0; nLinha < PQntuples(pgResultado); nLinha++)
  {
    for (nCol = 0; nCol < nQtdCol; nCol++) {
      ...
      if (PQftype(pgResultado, nCol) == 1114) //Timestamp
        sLinha[nCol] = Rotinas->ConverteDataHoraStr(sLinha[nCol], 
          false, "%d/%m/%Y", "%H:%M:%S");

      (*lstLocal).col[nCol].Add(sLinha[nCol]); 
    }      
  }
