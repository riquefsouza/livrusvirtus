...
bool CRotinasGlobais::ConectaBanco(wxString sStrConexao)
{
  wxString sTemp;
  ...
  try 
  {
    if FAILED( gConexao.CreateInstance("ADODB.Connection") ) {
      wxMessageBox(STR_NCCINST, wxT("Erro"), wxOK | wxICON_ERROR);
      return false;
    }

    gConexao->Open(sStrConexao.c_str(), 
      "", "", adConnectUnspecified);
  }
  catch (_com_error &e)
  {
    sTemp = wxString::Format("%s - %s", STR_NCONBANCO, 
...
bool CRotinasGlobais::ConsultaDados(CONEXAO pConexao, wxString sSql)
{
  wxString sTemp;
  _RecordsetPtr pRS;
...
bool CRotinasGlobais::AtualizaDados(CONEXAO pConexao, wxString sSql)
{
  wxString sTemp;
  _CommandPtr  pCmnd;
  ...
  {
    sTemp.Format("%s - %s", STR_NCEXECSQL, 
      e.Description().operator const char *());
    wxMessageBox(sTemp, wxT("Erro"), wxOK | wxICON_ERROR);
    return false;
  }   
