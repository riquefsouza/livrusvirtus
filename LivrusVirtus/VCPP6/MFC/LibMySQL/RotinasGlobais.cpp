...
{ CString sTemp;
  ...
  if ((gConexao = mysql_init((MYSQL*) 0)) && 
       mysql_real_connect( gConexao, "localhost", 
         sLogin.operator LPCTSTR(), sSenha.operator LPCTSTR(), NULL, MYSQL_PORT, NULL, 0 ) )
  {
    if ( mysql_select_db( gConexao, sBancoDeDados.operator LPCTSTR() ) < 0 ) {
    ...
    sTemp.Format("%s: %d",Rotinas->lerStr(STR_NCONPORTA), MYSQL_PORT);
...
BOOL CRotinasGlobais::ConsultaDados(CONEXAO mConLocal, CString sSql)
{
BOOL bRet;
MYSQL_RES	*mResultado;
  
  if ( mysql_query(mConLocal, sSql.operator LPCTSTR()) == 0 ) {
...
BOOL CRotinasGlobais::AtualizaDados(CONEXAO mConLocal, CString sSql)
{ 
  if ( mysql_query(mConLocal, sSql.operator LPCTSTR()) == 0)
    return TRUE;
  else {
