...
BOOL CRotinasGlobais::ConectaBanco(CString sStrConexao)
{
  CString sTemp;
  ...
  try 
  {
    if FAILED( gConexao.CreateInstance("ADODB.Connection") ) {
      AfxMessageBox(Rotinas->lerStr(STR_NCCINST), MB_ICONERROR | MB_OK, 0);
      return FALSE;
    }

    gConexao->Open(sStrConexao.operator LPCTSTR(), 
      "", "", adConnectUnspecified);
  }
  catch (_com_error &e)
  {
    sTemp.Format("%s - %s", Rotinas->lerStr(STR_NCONBANCO), 
...
BOOL CRotinasGlobais::ConsultaDados(CONEXAO pConexao, CString sSql)
{
  CString sTemp;
  _RecordsetPtr pRS;
  ...
  {    
    ...
    pRS->Open(sSql.operator LPCTSTR(), 
      _variant_t((IDispatch *) pConexao, true), 
  ...
  {
    sTemp.Format("%s - %s", Rotinas->lerStr(STR_NCEXECSQL), 
      e.Description().operator const char *());
    AfxMessageBox(sTemp, MB_ICONERROR | MB_OK, 0);
    return FALSE;
  }   
...
BOOL CRotinasGlobais::AtualizaDados(CONEXAO pConexao, CString sSql)
{
  CString sTemp;
  _CommandPtr  pCmnd;
  ...
  {    
    ...
    pCmnd->CommandText = (_bstr_t) sSql.operator LPCTSTR();
