...
{
CString sConexaoInfo;
  
  sConexaoInfo.Format("dbname = %s user = %s password = %s",
    sBancoDeDados, sLogin, sSenha);

  gConexao = PQconnectdb(sConexaoInfo.operator LPCTSTR());
...
BOOL CRotinasGlobais::ConsultaDados(CONEXAO pgConLocal, CString sSql)
{
  BOOL bRet;
  PGresult *pgResultado;
  CString sTemp;

  pgResultado = PQexec(pgConLocal, sSql.operator LPCTSTR());
...
BOOL CRotinasGlobais::AtualizaDados(CONEXAO pgConLocal, CString sSql)
{
  BOOL bRet;
  PGresult *pgResultado;
  CString sTemp;
