...
{ GString *sTemp = g_string_new("");
         ...
         sLogin, sSenha, NULL, MYSQL_PORT, NULL, 0 ) )
  {
    ...
    g_string_sprintf(sTemp, "%s: %d", STR_NCONPORTA, MYSQL_PORT);
...
gboolean Rotinas_ConsultaDados(CONEXAO mConLocal, GString *sSql)
{
gboolean bRet;
MYSQL_RES	*mResultado;
  
  if ( mysql_query(mConLocal, sSql->str) == 0 ) {
...
gboolean Rotinas_ConsultaDadosLista(CONEXAO mConLocal, 
  ...
  if ( mysql_query(mConLocal, sSql->str) == 0 ) {
    ...
    while ( mLinha ) {
      ...
      for (nCol = 0; nCol < nQtdCol; nCol++) {	  
        ...
        strcpy(sLinha[nCol], mLinha[nCol]);
...
gboolean Rotinas_AtualizaDados(CONEXAO mConLocal, GString *sSql) { 
gchar sLocal[TAM_MAX_STR * 4];

  strcpy(sLocal, Rotinas_retLocale(sSql->str));

  if ( mysql_query(mConLocal, sLocal) == 0)
    return TRUE;
  else {
    Rotinas_MsgDlg(STR_NCEXECSQL, GTK_MESSAGE_ERROR);
    return FALSE;
