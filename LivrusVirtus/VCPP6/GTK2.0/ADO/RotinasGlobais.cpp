...
gboolean Rotinas_ConectaBanco(gchar *sStrConexao)
{
  ...
  try 
  {
    if FAILED( gConexao.CreateInstance("ADODB.Connection") ) {
      Rotinas_MsgDlg(STR_NCCINST, GTK_MESSAGE_ERROR);
      return FALSE;
    }

    gConexao->Open(sStrConexao, "", "", adConnectUnspecified);
  }
  catch (_com_error &e)
  {
    g_string_sprintf(sTemp, "%s - %s", STR_NCONBANCO, 
...
gboolean Rotinas_ConsultaDados(CONEXAO pConexao, GString *sSql)
{
  ...
  {    
    ...
    pRS->Open(sSql->str, _variant_t((IDispatch *) pConexao, true), 
...
gboolean Rotinas_AtualizaDados(CONEXAO pConexao, GString *sSql)
{
  ...
  {    
    ...
    pCmnd->CommandText = (_bstr_t) sLocal;
      ...
      while (!pRS->adoEOF) {
        for (nCol = 0; nCol < nQtdCol; nCol++) {
          ...
          if (pRS->GetFields()->Item[nCol]->Type == adDBTimeStamp)
            strcpy(sLinha, Rotinas_ConverteDataHoraStr(sLinha, FALSE, 
              "%d/%m/%Y", "%H:%M:%S"));
          else if (pRS->GetFields()->Item[nCol]->Type == adSingle
            || pRS->GetFields()->Item[nCol]->Type == adDouble)
            strcpy(sLinha, Rotinas_VirgulaParaPonto(sLinha, false));

          strcpy(sAux, Rotinas_retUTF8(sLinha));
