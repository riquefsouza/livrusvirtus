...
uses
  ...
  Cadastros, Db, DBTables, StdCtrls, ComCtrls, ExtCtrls, ADODB;
...
if bInformar then begin
  edtEmail.Text:=Consulta.Fields[2].AsString;
  edtIdentidade.Text:=Consulta.Fields[3].AsString;
  if Consulta.Fields[4].AsString='M' then
     rdgSexo.ItemIndex:=0
  else
     rdgSexo.ItemIndex:=1;
  edtTelefone.Text:=Consulta.Fields[5].AsString;
  edtDtNascimento.DateTime:=Consulta.Fields[6].AsDateTime;
  edtCodEndereco.Text:=Consulta.Fields[7].AsString;
  edtLogradouro.Text:=Consulta.Fields[8].AsString;
  edtBairro.Text:=Consulta.Fields[9].AsString;
  edtCEP.Text:=Consulta.Fields[10].AsString;
  edtCidade.Text:=Consulta.Fields[11].AsString;
  edtEstado.Text:=Consulta.Fields[12].AsString;
  edtCodPais.Text:=Consulta.Fields[13].AsString;
  edtPais.Text:=Consulta.Fields[14].AsString;
  edtCodProfissao.Text:=Consulta.Fields[15].AsString;
  edtProfissao.Text:=Consulta.Fields[16].AsString;
end else begin
  edtEmail.Clear;
  edtIdentidade.Clear;
  rdgSexo.ItemIndex:=-1;
