#Boa:FramePanel:frmCadProfissoes
...
def create(parent):
    return frmCadProfissoes(parent)
...
class frmCadProfissoes(frmCadastros):
    ...
    def _init_ctrls(self, prnt):
        ...
        self._init_utils()
        self.labDescricao.SetLabel(u'Profiss\xe3o:')
    ...
    def ValidaDados(self, bTodosDados):
      ...
      if bTodosDados:
    	  if RotinasGlobais.Rotinas.ValidaCampo(
          self.edtDescricao.GetValue(), Resource.STR_PROFNINF):
      		return False;
      return True
    ...
    def OnBtnSalvarClick(self, event):
      self.SalvarDados(self.ValidaDados(True),
        ConsultasSQL.ConsSQL.Profissao('S', self.edtCodigo.GetValue(),""),
        ConsultasSQL.ConsSQL.Profissao('U', self.edtCodigo.GetValue(), 
        self.edtDescricao.GetValue()),
        ConsultasSQL.ConsSQL.Profissao('I', self.edtCodigo.GetValue(), 
        self.edtDescricao.GetValue()), True)
      event.Skip()

    def OnBtnExcluirClick(self, event):
      self.ExcluirDados(self.ValidaDados(False),
        ConsultasSQL.ConsSQL.Profissao('D', self.edtCodigo.GetValue(),""))
      event.Skip()
