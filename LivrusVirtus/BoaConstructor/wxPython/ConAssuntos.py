...
def create(parent):
    return frmConAssuntos(parent)

class frmConAssuntos(frmConsultas):
    def _init_ctrls(self, prnt):
      frmConsultas._init_ctrls(self, prnt)
      self.SetTitle(Resource.STR_CONASSUNTO)
      self.labDescricao.SetLabel("Assunto:  ")
      self.gridConsulta.SetColLabelValue(1, "Assunto")
      ...
      EVT_CLOSE(self, self.OnFrmConAssuntosClose)
    ...
    def OnFrmConAssuntosClose(self, event):
    ...
    def OnBtnPesquisarClick(self, event):
      self.PesquisarDados(
        ConsultasSQL.ConsSQL.Assunto('S', "", self.edtDescricao.GetValue()),
