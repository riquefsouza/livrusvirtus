...
class frmCadAutores(frmCadastros):
    ...
    def _init_ctrls(self, prnt):
        ...
        self._init_utils()
        self.labDescricao.SetLabel(u'Autor:')
    ...
    def OnBtnNovoClick(self, event):
      self.btnNovoClick()
      self.NovoDado(ConsultasSQL.ConsSQL.Autor('N', 
    ...
    def OnBtnSalvarClick(self, event):
      self.SalvarDados(self.ValidaDados(True),
        ConsultasSQL.ConsSQL.Autor('S', self.edtCodigo.GetValue(),""),
        ConsultasSQL.ConsSQL.Autor('U', self.edtCodigo.GetValue(), 
        self.edtDescricao.GetValue()),
        ConsultasSQL.ConsSQL.Autor('I', self.edtCodigo.GetValue(), 
        self.edtDescricao.GetValue()), True)
      event.Skip()

    def OnBtnExcluirClick(self, event):
      self.ExcluirDados(self.ValidaDados(False),
        ConsultasSQL.ConsSQL.Autor('D', self.edtCodigo.GetValue(),""))
      event.Skip()
