...
class frmCadAssuntos(frmCadastros):
    ...
    def _init_ctrls(self, prnt):
        ...
        self._init_utils()
        self.labDescricao.SetLabel(u'Assunto:')
    ...
    def ValidaDados(self, bTodosDados):
      ...
      if bTodosDados:
    	  if RotinasGlobais.Rotinas.ValidaCampo(
          self.edtDescricao.GetValue(), Resource.STR_ASSNINF):
      		return False;
      return True
    ...
    def OnBtnSalvarClick(self, event):
      self.SalvarDados(self.ValidaDados(True),
        ConsultasSQL.ConsSQL.Assunto('S', self.edtCodigo.GetValue(),""),
        ConsultasSQL.ConsSQL.Assunto('U', self.edtCodigo.GetValue(), 
        self.edtDescricao.GetValue()),
        ConsultasSQL.ConsSQL.Assunto('I', self.edtCodigo.GetValue(), 
        self.edtDescricao.GetValue()), True)
      event.Skip()

    def OnBtnExcluirClick(self, event):
      self.ExcluirDados(self.ValidaDados(False),
        ConsultasSQL.ConsSQL.Assunto('D', self.edtCodigo.GetValue(),""))
      event.Skip()
